...
export const DEFAULT_WEEK_KOR = ["일", "월", "화", "수", "목", "금", "토"];
...
export function getWeekDays(weeks: [string], firstDay: number): [string] {
  ...
  if (weeks === DEFAULT_WEEK_EN || weeks === DEFAULT_WEEK_ZH) {
    const temWeeks =
      weeks === DEFAULT_WEEK_EN ? DEFAULT_WEEK_EN : DEFAULT_WEEK_ZH;
    // Nothing changed
    if (firstDay === 0) {
      return temWeeks;
    }
...
export function getCalendarDates(
  ...
  for (let year = startYear; year <= endYear; year++) {
    if (startYear === endYear) {
      dates = dates.concat(
        constructCalendarDates(year, startMonth, endMonth, firstDay)
      );
    } else if (year === startYear) {
      dates = dates.concat(
        constructCalendarDates(year, startMonth, 12, firstDay)
      );
 ...
 * @returns {[]}
 */
function constructCalendarDates(
  year: number,
  startMonth: number,
  endMonth: number,
  firstDay: number
  ...
  for (let month = startMonth; month <= endMonth; month++) {
    ...
    weekDay =
      getWeekDay(`${year}-${month <= 9 ? "0" : ""}${month}-01`) - firstDay;
    if (weekDay < 0) {
      weekDay = weekDay + 7;
    }
...
export function getDatePickerInitialData(initialProps) {
  ...
  const _verifyDate = (date) => {
    ...
    const _printError = () =>
      console.warn(
        `The date type you selected is: ${type}, but has incorrect date format: ${aDate}`
      );
    switch (type) {
      case DATE_TYPE.YYYY:
      ...
      case DATE_TYPE.MM:
        __DEV__ &&
          (aDate.length > 2 || +aDate < 1 || +aDate > 12) &&
          _printError();
        return `${_y}-${aDate}-${_d}`;
      case DATE_TYPE.DD:
        __DEV__ &&
          (aDate.length > 2 || +aDate < 1 || +aDate > 31) &&
          _printError();
        return `${_y}-${_m}-${aDate}`;
      case DATE_TYPE.YYYY_MM:
      case DATE_TYPE.MM_YYYY:
  ...
  const monthIndex = months.findIndex(
    (item) =>
      item.date === getDatePickerMonth(monthDisplayMode, +_defaultDates[1])
  );
 ...
 */
function constructDatePickerDates(
  monthDisplayMode: string,
  startDate: string,
  endDate: string
) {
    ...
    for (let month = _startMonth; month <= _endMonth; month++) {
      ...
      if (year > startYear && year < endYear) {
        ...
        if (_startMonth === _endMonth) {
          _startDay =
            startYear === endYear || year === startYear ? startDay : 1;
...
function getDatePickerMonth(monthDisplayMode: string, month: number) {
  if (Object.values(MONTH_DISPLAY_MODE).indexOf(monthDisplayMode) < 0)
    return month;
...
export function greaterThan(date: string, another: string) {
  ...
  if (parseInt(dateArr[0]) === parseInt(anotherArr[0])) {
    ...
    if (parseInt(dateArr[1]) === parseInt(anotherArr[1]))
      return parseInt(dateArr[2]) > parseInt(anotherArr[2]); // return day1 > day2
  }
  return false;
}
 ...
 */
export function toStandardStringWith(
  date: string,
  monthDisplayMode: string = ""
): string {
  ...
  if (monthDisplayMode === MONTH_DISPLAY_MODE.EN_SHORT) {
    month =
      DEFAULT_MONTH_SHORT.indexOf(month) >= 0
        ? (DEFAULT_MONTH_SHORT.indexOf(month) + 1).toString()
        : month;
  }

  if (monthDisplayMode === MONTH_DISPLAY_MODE.EN_LONG) {
    month =
      DEFAULT_MONTH_LONG.indexOf(month) >= 0
        ? (DEFAULT_MONTH_LONG.indexOf(month) + 1).toString()
        : month;
  }
